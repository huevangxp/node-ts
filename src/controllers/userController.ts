...
export const createUser = async (req: Request, res: Response) => {
    try {
        ...
        //check user in db
        const checkUser = await results.findOne({ where: { username: req.body.username } })
        if (checkUser) {
        ...
        if (!saveUser) {
            return res.status(404).json({ message:'can not create user repository'})
        }
...
export const updateUser = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(200).json({ message:'Update this user', data: userUpdated})
...
export const login = async (req: Request, res: Response) => {
    try {
        ...
        const users = await AppDataSource.getRepository(User).findOne({ where: { username: username } })
        if (!users) {
        ...
        const token = await jwt.sign(userToken, "huevang-Test-node*ts", { expiresIn:'24h'})

        if (!token) {
            return res.status(401).json({ message:"token not valid"})
        }

        return res.status(200).json({message:'login success',data:userToken, token: token})
    } catch (error) {
        return res.status(500).json({ message:error})
    }
}